...
class Operate:
    ...
    def get_next_instruction(self):
        ...
        """    
        stop_time = 5
        if not self.checkpoints: # Nothing in self.checkpoints
            ...
            try:
                ...
                pass
        if not self.checkpoints[0]: # Completed set
            print("Cleaning up")
    ...
    def draw(self, canvas):
        ...
        ekf_view = self.ekf.draw_slam_state(self.fruit_true_pos, self.fruit_list,checkpoints=self.checkpoints,shopping_list=self.search_list, res=(600,600+v_pad),
    ...
    def navigate_to_next(self):
        ...
        waypoint = fruit_true_pos[self.fruit_list.index(self.search_list[0])]
        ...
        for radius in safe_radiuses:                
            ...
            start = self.get_robot_state()[:2].squeeze().tolist()
            print(start)
            for _ in range(10):
    ...
    def get_object_poses(self):
        if self.detector is not None:
            object_poses = {} # Relative poses
            for obj_info in self.detector_output:
                obj_relative_pos = estimate_object_pose(self.camera_matrix,obj_info)
                if obj_info[0] in object_poses.keys():
                    if np.hypot(obj_relative_pos['x'],obj_relative_pos['y']) < np.hypot(object_poses[obj_info[0]]['x'],object_poses[obj_info[0]]['y']): # new target closer than existing targt
                        object_poses[obj_info[0]] = obj_relative_pos
        return object_poses
...
if __name__ == "__main__":
    ...
    parser.add_argument("--search_list", type = str, default="testing_shopping_list.txt")
    parser.add_argument("--yolo_model", default='YOLO/model/weights/200_Epochs.pt')
    # parser.add_argument("--yolo_model", default='YOLO/model/weights/150_Epochs.pt')
    ...
    width, height = 700+280, 660+120+20 # modified GUI due to larger slam map
